"""Probe DeepInfra Gemma responses for Padrino's agent contract."""
...
_MODEL_ID = "deepinfra/google/gemma-4-26B-A4B-it"
...
def _canonical_observation() -> Observation:
    ...
    state = GameState(
        ...
        game_id="G-DEEPINFRA-GEMMA-PROBE",
        game_seed="seed-deepinfra-gemma-probe",
        current_phase=phase,
        seats=_SEATS,
        day=phase.day,
    )
...
def main() -> None:
    try:
        ...
        raise SystemExit(f"DEEPINFRA_API_KEY is not configured: {exc}") from exc
